...
def save_path(local=False):
    ...
    if path == plugin.get_setting('temp_path', unicode):
...
def temp_path():
    path = ensure_path_local(plugin.get_setting('temp_path', unicode))
    if not xbmcvfs.mkdir(path):
        raise LocalizedError(33030, "Invalid temporary path: " + path + " --", check_settings=True)
    return path
